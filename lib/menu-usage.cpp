 ...
 * @LastEditTime : 2020-06-05 08:33:20
 * @Description  :
...
MenuUsage::MenuUsage()
{
    ...
    this->session_proxy = Gio::DBus::Proxy::create_sync(session_bus,
                                                        "org.gnome.SessionManager",
                                                        "/org/gnome/SessionManager/Presence",
                                                        "org.gnome.SessionManager");

    this->session_proxy->signal_signal().connect(sigc::mem_fun(this, &MenuUsage::session_proxy_signal));
}
...
void MenuUsage::active_window_changed(WnckScreen *screen, WnckWindow *previously_active_window)
{
    ...
    if (this->focus_desktop_id.length() > 0)
    {
        increment_usage_for_app_at_time(this->focus_desktop_id, cur_system_time);
        this->focus_desktop_id.clear();
    }
    ...
    if (app)
    {
        this->focus_desktop_id = app->get_desktop_id();
        auto &usage_data = get_usage_for_app(this->focus_desktop_id);
    ...
    this->watch_start_time = cur_system_time;
}
...
std::vector<std::string> MenuUsage::get_nfrequent_apps(gint top_n)
{
    ...
    for (auto iter = this->app_usages.begin(); iter != this->app_usages.end(); ++iter)
    {
    ...
    std::sort(apps.begin(), apps.end(), [this](const std::string &a, const std::string &b) -> bool {
        ...
        auto itera = this->app_usages.find(quarka.id());
        auto iterb = this->app_usages.find(quarkb.id());

        double a_score = (itera == this->app_usages.end()) ? 0 : itera->second.score;
        double b_score = (iterb == this->app_usages.end()) ? 0 : iterb->second.score;
...
void MenuUsage::on_session_status_changed(uint32_t status)
{
    ...
    if (this->screen_idle == idle)
        return;

    this->screen_idle = idle;
    if (idle)
    {
        if (this->focus_desktop_id.length() > 0)
        {
            auto end_time = this->watch_start_time + IDLE_TIME_TRANSITION_SECONDS;
            increment_usage_for_app_at_time(this->focus_desktop_id, end_time);
        }
    }
    else
    {
        this->watch_start_time = get_system_time();
    }
}
...
MenuUsage::UsageData &MenuUsage::get_usage_for_app(const std::string &desktop_id)
{
    ...
    return this->app_usages[quark.id()];
}
...
bool MenuUsage::write_usages_to_settings()
{
    ...
    return this->settings->set_value("frequent-apps", Glib::Variant<std::vector<UsagePair>>::create(apps));
}

bool MenuUsage::read_usages_from_settings()
{
    Glib::VariantBase base;
    this->settings->get_value("frequent-apps", base);
    ...
    try
    {
        ...
        for (gsize i = 0; i < child_num; ++i)
        {
            ...
            Glib::Quark quark(desktop_id);
            auto iter = this->app_usages.emplace(quark.id(), UsageData{score, last_seen});
            if (!iter.second)
            {
                return false;
            }
...
void MenuUsage::ensure_queued_save()
{
    ...
    this->save_id = timeout.connect_seconds(sigc::mem_fun(this, &MenuUsage::write_usages_to_settings), SAVE_APPS_TIMEOUT_SECONDS);
}
...
void MenuUsage::increment_usage_for_app_at_time(const std::string &desktop_id, int32_t time)
{
    ...
    guint elapsed = time - this->watch_start_time;
