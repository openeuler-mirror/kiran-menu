...
WorkspaceWindowsOverview::WorkspaceWindowsOverview() : old_allocation(-1, -1, 0, 0),
...
void WorkspaceWindowsOverview::on_size_allocate(Gtk::Allocation &allocation)
{
    ...
    if (old_allocation.get_width() != allocation.get_width() || old_allocation.get_height() != allocation.get_height())
    {
        ...
        if (reload_handler.connected())
        {
            KLOG_DEBUG("workspace overview allocate, reloading thumbnails already scheduled");
            return;
        }

        KLOG_DEBUG("workspace overview allocate %d x %d, schedule for reloading thumbnails",
                   allocation.get_width(),
                   allocation.get_height());
        clear();
...
void WorkspaceWindowsOverview::reload_thumbnails()
{
    ...
    KLOG_DEBUG("viewport size %d x %d, rows %d",
               viewport_width,
               viewport_height,
               rows);
    ...
    for (auto row : results)
    {
        ...
        KLOG_DEBUG("row[%d], real scale %.2lf, x_scale %.2lf, y_scale %.2lf, max_height %d, sum %d\n",
                                             ...
                                             int viewport_height)
{
    ...
    if (rows > max_rows)
    {
        KLOG_WARNING("No valid rows count found, use max_rows %d", max_rows);
        rows = max_rows;
    }
    return rows;
}