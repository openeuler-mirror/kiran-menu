...
static gboolean handle_add_category_app(KiranStartMenuS *skeleton,
                                           ...
                                           KiranAppCategory *self) {
  GVariant *category_apps = g_hash_table_lookup(self->category_apps, category);
  GVariantBuilder builder;
  ...
  if (category_apps) {
    ...
    for (gsize i = 0; i < child_num; ++i) {
      ...
      g_variant_get_child(category_apps, i, "s", &elem_desktop_file);
  ...
  if (match_app) {
    ...
    g_hash_table_insert(self->category_apps, category, g_variant_builder_end(&builder));
    write_category_apps_data(self);
  }
