...
TasklistButtonsContainer::TasklistButtonsContainer(MatePanelApplet *applet_, int spacing_) : Glib::ObjectBase("KiranTasklistButtonsContainer"),
                                                                                             applet(applet_),
...
void TasklistButtonsContainer::add_app_button(const KiranAppPointer &app)
{
    ...
    KLOG_DEBUG("add button for app '%s'", app->get_desktop_id().c_str());
}
...
void TasklistButtonsContainer::remove_app_button(const KiranAppPointer &app)
{
    ...
    if (!button)
    {
        KLOG_WARNING("no button for app '%s'", app->get_name().data());
        return;
    }
...
void TasklistButtonsContainer::on_window_opened(KiranWindowPointer window)
{
    ...
    if (!app)
    {
        KLOG_WARNING("no app found for new window '%s'\n", window->get_name().data());
        return;
    }
    ...
    {
        KLOG_DEBUG("app button already exists\n");
...
void TasklistButtonsContainer::move_previewer(TasklistAppButton *target_button)
{
    ...
    {
        KLOG_DEBUG("target app expired or has no windows\n");
        previewer->hide();
        return;
    }
...
void TasklistButtonsContainer::on_applet_size_change()
{
    ...
    KLOG_DEBUG("applet size changed to %d", applet_size);
...
void TasklistButtonsContainer::get_preferred_width_vfunc(int &min_width, int &natural_width) const
{
    ...
    //KLOG_INFO("container min %d, natural %d\n", min_width, natural_width);
}
        ...
        try
        {
            ...
            if (source_app)
            {
                ...
                else
                {
                    ...
                    KLOG_WARNING("NEW user app created, id '%s'", new_id.c_str());
                    if (!new_id.empty())
                    {
                        Glib::signal_idle().connect_once(
                            [new_id]() -> void {
                ...
                KLOG_WARNING("Failed to add fixed apps: can not create app for desktop file '%s'", uri.c_str());
        }
        catch (const Gio::Error &e)
        {
            KLOG_WARNING("Failed to query information for '%s'", uri.c_str());
            continue;
        }
    }
}
...
void TasklistButtonsContainer::load_applications()
{
    ...
    for (auto child : get_children())
    {
        ...
        if (old_app->get_kind() != Kiran::AppKind::FAKE_DESKTOP)
        {
            ...
            if (new_app)
            {
                ...
                {
                    KLOG_DEBUG("update button for app '%s'", old_app->get_desktop_id().c_str());
                ...
                {
                    /* 应用未卸载，但窗口已归属到其它的应用 */
                    KLOG_DEBUG("remove button for app '%s', since no windows opened", old_app->get_desktop_id().c_str());
                }
            }
            else
            {
                /* 已卸载应用 */
                KLOG_DEBUG("remove button for uninstalled app '%s'", old_app->get_desktop_id().c_str());
            }
        }
        else
        {
            /* Fake desktop 应用 */
            KLOG_DEBUG("remove button for fake app '%s'", old_app->get_desktop_id().c_str());
        }
    ...
    //加载常驻任务栏应用
    KLOG_DEBUG("%s: loading fixed apps ...\n", __FUNCTION__);
    ...
    //加载当前运行应用
    KLOG_DEBUG("%s: loading running apps ...\n", __FUNCTION__);
    if (backend->get_app_show_policy() == Kiran::TaskBarSkeleton::POLICY_SHOW_ALL)
    {
...
{
    KLOG_DEBUG("Got fixed apps added signal\n");
    for (auto app : apps)
    {
        KLOG_DEBUG("add fixed app '%s'", app->get_name().data());
...
{
    KLOG_DEBUG("Got fixed apps removed signal\n");
    for (auto app : apps)
    {
        KLOG_DEBUG("remove fixed app '%s', %d windows found\n",
                   app->get_name().data(),
...
void TasklistButtonsContainer::switch_to_page_of_button(TasklistAppButton *button)
{
    ...
    {
        KLOG_WARNING("button not realized, allocation (%d, %d), %d x %d",
            ...
            if (KiranHelper::gdk_rectangle_contains_point(rect.gobj(), &point))
            {
                ...
                if (child != widget)
                {
                    ...
                    case MOTION_DIR_DOWN:
                        ...
                        break;
                    default:
                        KLOG_WARNING("shouldn't get here");
                        break;
                    }
                }
                found = true;
