...
{
    TasklistAppletWidget *widget;

    widget = static_cast<TasklistAppletWidget*>(userdata);
    ...
    applet(applet_)
{
    ...
    window_manager->signal_active_window_changed().connect(
                sigc::mem_fun(*this, &TasklistAppletWidget::on_active_window_changed));
    window_manager->signal_window_opened().connect(
                sigc::mem_fun(*this, &TasklistAppletWidget::on_window_opened));
    window_manager->signal_window_closed().connect(
                sigc::mem_fun(*this, &TasklistAppletWidget::on_window_closed));

    //响应常住任务栏应用变化信号
    auto backend = Kiran::TaskBarSkeleton::get_instance();
    backend->signal_fixed_app_added().connect(
                sigc::mem_fun(*this, &TasklistAppletWidget::on_fixed_apps_added));
    backend->signal_fixed_app_deleted().connect(
                sigc::mem_fun(*this, &TasklistAppletWidget::on_fixed_apps_removed));

    auto menu_backend = Kiran::MenuSkeleton::get_instance();
    menu_backend->signal_app_changed().connect(sigc::mem_fun(*this, &TasklistAppletWidget::reload_app_buttons));
    ...
    previewer->signal_opened().connect(
                sigc::mem_fun(*this, &TasklistAppletWidget::on_previewer_window_opened));

    //Schedule: 加载应用按钮
    Glib::signal_idle().connect_once(
                sigc::mem_fun(*this, &TasklistAppletWidget::load_applications));

    get_style_context()->add_class("tasklist-widget");
}

TasklistAppletWidget::~TasklistAppletWidget()
{
    delete previewer;
}
 ...
 * @param app 待添加的应用
 */
void TasklistAppletWidget::add_app_button(const KiranAppPointer &app)
{
    if (!app || find_app_button(app))
        return;

    auto button = Gtk::manage(new TasklistAppButton(app));
    button->show_all();
#if 1
    //鼠标进入应用按钮时，显示预览窗口
                ...
                sigc::bind_return<bool>(
                    sigc::hide(
                        sigc::bind<TasklistAppButton*>(
                            sigc::mem_fun(*this, &TasklistAppletWidget::move_previewer),
                            button)),
                    false));
    ...
    button->signal_leave_notify_event().connect(
                sigc::bind_return<bool>(
                    sigc::hide(sigc::mem_fun(*this, &TasklistAppletWidget::hide_previewer)),
                    false));
#endif
...
void TasklistAppletWidget::toggle_previewer(TasklistAppButton *button)
{
 ...
 *
 */
void TasklistAppletWidget::remove_app_button(const KiranAppPointer &app)
{
    if (!app)
        return;
 ...
 */
KiranAppPointer TasklistAppletWidget::get_current_active_app()
{
 ...
 */
TasklistAppButton *TasklistAppletWidget::find_app_button(
...
void TasklistAppletWidget::on_active_window_changed(KiranWindowPointer previous,
...
void TasklistAppletWidget::on_window_opened(KiranWindowPointer window)
{
...
void TasklistAppletWidget::on_window_closed(KiranWindowPointer window)
{
    auto app = window->get_app();
    if (!app)
        return;
 ...
 */
void TasklistAppletWidget::move_previewer(TasklistAppButton *target_button)
{
...
void TasklistAppletWidget::hide_previewer()
{
 ...
 */
void TasklistAppletWidget::update_orientation()
{
    if (!applet)
        return;
...
void TasklistAppletWidget::get_preferred_width_vfunc(int &min_width, int &natural_width) const
{
...
void TasklistAppletWidget::get_preferred_height_vfunc(int &min_height, int &natural_height) const
{
    unsigned int applet_size = mate_panel_applet_get_size(applet);
...
void TasklistAppletWidget::on_size_allocate(Gtk::Allocation &allocation)
{
    Gtk::Requisition min, natural;
...
void TasklistAppletWidget::reload_app_buttons()
{
 ...
 */
void TasklistAppletWidget::load_applications()
{
    Kiran::AppVec apps;
 ...
 * @param apps
 */
void TasklistAppletWidget::on_fixed_apps_added(const Kiran::AppVec &apps)
{
...
void TasklistAppletWidget::on_fixed_apps_removed(const Kiran::AppVec &apps)
{
...
void TasklistAppletWidget::on_previewer_window_opened()
{
...
void TasklistAppletWidget::handle_applet_size_change(int size)
{
    queue_resize();
}

void TasklistAppletWidget::handle_applet_orient_change(int orient)
{
